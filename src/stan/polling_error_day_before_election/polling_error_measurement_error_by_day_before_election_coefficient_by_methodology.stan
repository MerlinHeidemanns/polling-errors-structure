...
transformed parameters {
  ...
  matrix[21, P] inv_logit_alpha;
  matrix[21, P] m1_inv_logit_alpha;
  xi = xi_sigma * raw_xi;
  for (tt in 1:T){
    xi_national[tt] = logit(turnout_weights[tt] * inv_logit(xi[(1 + (tt - 1) * S):(tt * S)]));
  }
  for (pp in 1:P){
    inv_logit_alpha[,pp] = inv_logit(mu[pp] + beta[pp] * seq);
  }
...
model {
  ...
  for (nn in 1:N_state){
    y_state[nn] ~ binomial(n_state[nn], inv_logit_alpha[t_state_before_election[nn], p_state[nn]] .* inv_logit(logit_outcome_state[nn] + xi[x[nn]]) +
      (1 - inv_logit_alpha[t_state_before_election[nn], p_state[nn]]) * 0.5);
  }
  for (nn in 1:N_national){
    y_national[nn] ~ binomial(n_national[nn], inv_logit_alpha[t_national_before_election[nn], p_national[nn]] .* inv_logit(logit_outcome_national[nn] +
      xi_national[t_national[nn]]) + (1 - inv_logit_alpha[t_national_before_election[nn], p_national[nn]]) * 0.5);
  }
}
