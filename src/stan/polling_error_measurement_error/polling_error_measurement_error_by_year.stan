...
transformed parameters {
  ...
  for (tt in 1:T){
    xi[8 + (tt - 1) * S] = 0.0;
    xi_national[tt] = logit(turnout_weights[tt] * inv_logit(logit(outcome_state[(1 + (tt - 1) * S):(tt * S)]) + xi[(1 + (tt - 1) * S):(tt * S)]));
  }
}
model {
  lambda ~ beta(2, 2);
  ...
  y_state ~ binomial(n_state, (1 - lambda[t_state]) .* inv_logit(logit_outcome_state + xi[x]) +
    lambda[t_state] * 0.5);
  y_national ~ binomial(n_national, (1 - lambda[t_national]) .* inv_logit(xi_national[t_national]) +
    lambda[t_national] * 0.5);
}
