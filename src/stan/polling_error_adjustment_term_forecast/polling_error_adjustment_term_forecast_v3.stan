...
model {
  ...
  y_state_past ~ binomial_logit(n_state_past,
    logit((1 - lambda[t_state_past]) .* inv_logit(logit(outcome_state[x_past])) +
    lambda[t_state_past] * 0.5) +
     + xi[x_past] + xi_national[t_state_past]);
  y_state_current ~ binomial_logit(n_state_current, beta[s_current] +
                                                          xi[x_current] +
                                                          xi_national[T]);
}
generated quantities {
  ...
  //lambda_new = normal_rng(mean(lambda), sqrt(variance(lambda)/2));
  lambda_new = uniform_rng(min(lambda), max(lambda));
  beta_new   = logit((inv_logit(beta) - lambda_new * 0.5) / (1 - lambda_new));

  y_hat[:, T] = inv_logit(beta_new +
                                xi[((T - 1) * S + 1):(T * S)] +
                                xi_national[T]);
  pred_new = inv_logit(beta_new + xi[((T - 1) * S + 1):(T * S)] + xi_national[T]);
