...
simulate_polls <- function(N, respondents, true_data, alpha, P){
  ...
  theta <- rep(NA, N)
  theta <- (1 - alpha) * inv.logit(logit(true_data$mu_b[s_poll]) + xi[s_poll] + mu_c[p_poll]) + alpha * 0.5
  n <- rep(respondents, N)
  y <- rbinom(N, n, theta)
  df <- data.frame(
    s = s_poll,
...
## polls
polls <- simulate_polls(300, 1000, true_data, 0.2, 20)
## datalist
data_list <- prepare_data_list(polls$df, true_data)
data_list[["prior_xi_sigma"]] <- 1
## model
mod <- cmdstan_model(
  "code/stan/models/models_polls/components/polling_error_measurement_error/simulation/base_model_w_mu_c_pp.stan")
## fit
fit <- mod$sample(
  data = data_list,
  seed = 123,
