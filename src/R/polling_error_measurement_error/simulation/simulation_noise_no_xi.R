...
simulate_polls <- function(N, respondents, true_data, alpha){
  ...
  theta <- rep(NA, N)
  # theta <- inv.logit(logit(true_data$mu_b[s_poll]) + xi[s_poll])
  ...
  #   y = y1 + y2
  # )
  theta <- (1 - alpha) * true_data$mu_b[s_poll] + alpha * 0.5
  n <- rep(respondents, N)
  y <- rbinom(N, n, theta)
  df <- data.frame(
    s = s_poll,
...
## model
mod <- cmdstan_model(
  "code/stan/models/models_polls/components/polling_error_measurement_error/simulation/base_model_no_xi.stan")
## fit
fit <- mod$sample(
  data = data_list,
  seed = 123,
