...
ggsave("plt/polling_error_voteshare/pres_polling_error_pres_voteshare_general.png",
       plt,
       height = 6, width = 10)
plt <- ggplot(data = pred, aes(x = previous_rep_share, y = q50)) +
  geom_line(size = 0.5) +
  ...
  labs(caption = "Positive polling errors underestimate Republican support
       ...
       x = "Republican vote share in previous election",
       y = "Polling error (%, favors Democrats)") +
  xlim(c(min(state_error$previous_rep_share),
         max(state_error$previous_rep_share)))
## all
...
plt <- ggplot(data = pred, aes(x = previous_rep_share, y = q50)) +
  ...
  labs(caption = "Positive polling errors underestimate Republican support
       ...
       y = "Polling error (%, favors Democrats)") +
  xlim(c(min(state_error$previous_rep_share),max(state_error$previous_rep_share)))
ggsave("plt/polling_error_voteshare/pres_polling_error_pres_voteshare_all.png",
       plt,
       height = 6, width = 10)
