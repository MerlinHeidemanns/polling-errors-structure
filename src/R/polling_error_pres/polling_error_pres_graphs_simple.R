...
library(rstanarm)
## Fit
fit <- read_rds("dta/model_output/polling_error_pres_simple.RDS")
## Data
results <- read_csv("dta/potus_results_76_20.csv") %>%
  mutate(dem_share = dem/(dem + rep)) %>%
  select(year, state_po, dem_share)
state_abb <- read_csv("dta/clean_data/polls_pres_prepared.csv") %>%
  distinct(s, state) %>%
  arrange(s) %>%
  pull(state)
...
fit <- stan_glm(q50 ~ previous_rep_share * t, data = state_error)
new <- data.frame(
  t = as.factor(sort(rep(seq(2000, 2020, 4), 100))),
...
ggsave("plt/polling_error_voteshare/pres_polling_error_pres_voteshare_general_simple.png",
       plt,
       height = 6, width = 10)

plt <- ggplot(data = pred, aes(x = previous_rep_share, y = q50)) +
  geom_line() +
  geom_text(aes(x = previous_rep_share, y = q50, label = s), size = 2, data = state_error) +
  ...
  labs(caption = "Positive polling errors underestimate Republican support
       ...
       y = "Polling error (%, favors Democrats)") +
  xlim(c(min(state_error$previous_rep_share),max(state_error$previous_rep_share)))
ggsave("plt/polling_error_voteshare/pres_polling_error_pres_voteshare_general_simple_labels.png",
       plt,
       height = 6, width = 10)
...
plt <- ggplot(data = pred, aes(x = previous_rep_share, y = q50)) +
  ...
  labs(caption = "Positive polling errors underestimate Republican support
       ...
       y = "Polling error (%, favors Democrats)") +
  xlim(c(min(state_error$previous_rep_share),max(state_error$previous_rep_share)))
ggsave("plt/polling_error_voteshare/pres_polling_error_pres_voteshare_all_simple.png",
       plt,
       height = 6, width = 10)
