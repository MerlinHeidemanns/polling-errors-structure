...
m <- file.path("code/stan/obs/input/polling_error/polling_error_additive_ar",
               "polling_error_additive_ar_hierarchical_prior_rho.stan")
mod <- cmdstan_model(m)
...
write_rds(prediction_df,
          file = "data/us_input/polling_error/polling_error_additive_ar_hierarchical_predict_loo.RDS")
...
### Compare
loo_pred <- read_rds(file = "data/us_input/polling_error/polling_error_additive_ar_hierarchical_predict_loo.RDS")
loo_pred <- loo_pred %>%
  as_tibble() %>%
...
plt <- ggplot(full_est %>% filter(s %in% state_abb[1:10], draw <= 500),
  ...
  labs(x = "Full model",
       y = "Loo prediction")
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/hierarchical_prediction_hist_states.png",
       height = 6, width = 10)
...
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/hierarchical_prediction_diff_hist_year.png",
       height = 6, width = 10)
...
plt <- ggplot(data = diff_df, aes(x = s, y = q50)) +
  ...
  labs(y = "Polling error estimate error (percentage points)",
       caption = "Positive: Underestimate
                  Negative: Overestimate")
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/hierarchical_prediction_diff_lines_year_states.png",
...
plt <- ggplot(data = quantile_df_sum,
  ...
  labs(x = "Ideal share",
       ...
       color = "Election") +
  theme_light()
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/hierarchical_calibration.png",
       plt ,
       height = 6, width = 10)
