...
m <- file.path("code/stan/obs/input/polling_error/polling_error_additive_ar",
               "polling_error_additive_ar_cov_mat.stan")
mod <- cmdstan_model(m)


prediction_df <- data.frame()
seq_miss_t <- c(2008, 2012, 2016, 2020)
for (miss_t in seq_miss_t){
  print(miss_t)
  obs <- (miss_t - 2000)/4
...
## save
write_rds(prediction_df,
          file = "data/us_input/polling_error/polling_error_additive_ar_cov_mat_predict_loo.RDS")
...
### Compare
loo_pred <- read_rds(file = "data/us_input/polling_error/polling_error_additive_ar_cov_mat_predict_loo.RDS")
loo_pred <- loo_pred %>%
  as_tibble() %>%
...
plt <- ggplot(full_est %>% filter(s %in% state_abb[1:10], draw <= 500),
  ...
  labs(x = "Full model",
       y = "Loo prediction")
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/cov_mat_prediction_hist_states.png",
       height = 6, width = 10)
...
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/cov_mat_prediction_diff_hist_year.png",
       height = 6, width = 10)
...
plt <- ggplot(data = diff_df, aes(x = s, y = q50)) +
  ...
  labs(y = "Polling error estimate error (percentage points)",
       caption = "Positive: Underestimate
                  Negative: Overestimate")
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/cov_mat_prediction_diff_lines_year_states.png",
...
plt <- ggplot(data = quantile_df_sum,
              aes(x = quantile, y = share_in, color = as.factor(t))) +
  geom_line() +
  ...
  labs(x = "Ideal share",
       ...
       color = "Election") +
  theme_light()
ggsave("plots/input/polling_error/polling_error_additive_predict_ar/cov_mat_calibration.png",
       plt,
       height = 6, width = 10)
...
loo_pred <- read_rds(file = "data/us_input/polling_error/polling_error_additive_ar_cov_mat_predict_loo.RDS")
loo_pred_wide <- loo_pred %>%
  group_by(s, missing) %>%
  mutate(draw = 1:n(),
         s = state_abb[s],
         s = factor(s, levels = states_2020_ordered)) %>%
  arrange(s) %>%
  pivot_wider(id_cols = c(missing, draw),
              values_from = val,
              names_from = s) %>%
  select(-draw) %>%
  ungroup()
...
for (i in 1:4){
  year = 2004 + i * 4
  cor_df <- loo_pred_wide %>%
    filter(missing == year) %>%
    select(-missing) %>%
    as.matrix()
  plt_list[[i]] <- ggcorrplot::ggcorrplot(cor(cor_df))
}
